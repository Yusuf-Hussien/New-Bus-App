...
namespace NewBusDAL.Driver.Mapper
{
    public class DriverProfile:Profile
    {
        public DriverProfile()
        {
            ...
            CreateMap<NewBusDAL.Models.Driver, DTODriverRead>()
       ...
       ForMember(to => to.UserName, from => from.MapFrom(from => from.Username)).
              ForMember(to => to.PlateNoBus, from => from.MapFrom(from => from.Bus.PlateNo)).
                            ForMember(to => to.CreatedByAdminName, from => from.MapFrom(from => from.Admin.Person.FirstName))
