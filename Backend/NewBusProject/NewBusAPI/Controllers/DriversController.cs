...
namespace NewBusAPI.Controllers
{
    ...
    public class DriversController : ControllerBase
    {
        ...
        public DriversController(IDriverBLL driverBLL)
        {
            _driverBLL = driverBLL;
        }
        [HttpGet("GetCurrentLocation/{id}")]
        public async Task<ActionResult<ApiResponseSucess<DTOCurrentLocation>>> GetCurrentLocation([FromRoute] int id)
        {
            var driver = await _driverBLL.GetCurrentLocation(id);
            return Ok(new ApiResponseSucess<DTOCurrentLocation>(driver, "Driver Data By ID"));
        }

        [Authorize(Roles = "Admin")]
        [HttpGet("{id}")]
        public async Task<ActionResult<ApiResponseSucess<DTODriverRead>>> GetDriverById([FromRoute] int id)
        {
            var driver = await _driverBLL.GetDriverByID(id);
            return Ok(new ApiResponseSucess<DTODriverRead>(driver, "Driver Data By ID"));
        }


        [Authorize(Roles = "Admin")]
        [HttpGet("getbyFirstName/{FirstName}")]
        public async Task<ActionResult<ApiResponseSucess<IEnumerable<DTODriverRead>>>> GetDriverByFirstName([FromRoute] string FirstName)
        {
            var driver = await _driverBLL.GetDriverByFirstName(FirstName);
            return Ok(new ApiResponseSucess<IEnumerable<DTODriverRead>>(driver, "Drivers Data By FirstName"));

        }

        [Authorize(Roles = "Admin")]
        [HttpGet("getbyLastName/{LastName}")]
        public async Task<ActionResult<ApiResponseSucess<IEnumerable<DTODriverRead>>>> GetDriverByLastName([FromRoute] string LastName)
        {
            var driver = await _driverBLL.GetDriverByLastName(LastName);
            return Ok(new ApiResponseSucess<IEnumerable<DTODriverRead>>(driver, "Drivers Data By LastName"));
        }


        [Authorize(Roles = "Admin")]
        [HttpGet("getbyPhone/{Phone}")]
        public async Task<ActionResult<ApiResponseSucess<DTODriverRead>>> GetDriverByPhone([FromRoute] string Phone)
        {
            var driver = await _driverBLL.GetDriverByPhone(Phone);
            return Ok(new ApiResponseSucess<DTODriverRead>(driver, "Driver Data By Phone"));
        }

        [Authorize(Roles = "Admin")]
        [HttpGet("GetByUsername/{username}")]
        public async Task<ActionResult<ApiResponseSucess<DTODriverRead>>> GetDriverByUsername([FromRoute] string username)
        {
            var driver = await _driverBLL.GetDriverByUsername(username);
            return Ok(new ApiResponseSucess<DTODriverRead>(driver, "Driver Data By Username"));
        }

        [Authorize(Roles = "Admin")]
        [HttpGet]
        public async Task<ActionResult<ApiResponseSucess<IEnumerable<DTODriverRead>>>> GetAllDrivers()
        {
            var drivers = await _driverBLL.GetAllDriver();
            return Ok(new ApiResponseSucess<IEnumerable<DTODriverRead>>(drivers, "Drivers Data"));
        }


        [Authorize(Roles = "Admin")]
        [HttpDelete("{id}")]
        public async Task<ActionResult<ApiResponseSucess<string>>> DeleteDriverById([FromRoute] int id)
        {
            await _driverBLL.DeleteDriver(id);
            return Ok(new ApiResponseSucess<string>("", "Driver Deleted Successfully"));
        }
        [Authorize(Roles = "Admin")]
        [HttpPost("SignUp")]
        public async Task<ActionResult<ApiResponseSucess<string>>> CreateDriver([FromBody] DTODriverCreate dtoDriverCreate)
        {
            ...
            return Ok(new ApiResponseSucess<string>("", "Verifiy Your Account Through Your Email"));
        }

        [Authorize(Roles = "Driver")]
        [HttpPut("Updatedriver")]
        public async Task<ActionResult<ApiResponseSucess<string>>> UpdateDriver([FromBody] DtoDriverUpdate dtoDriverUpdate)
        {
            ...
            return Ok(new ApiResponseSucess<string>("", "Driver Updated Successfully"));
        }

        [Authorize(Roles = "Admin")]
        [HttpPut("UpdateDriverforBus")]
        public async Task<ActionResult<ApiResponseSucess<string>>> UpdateDriverBus([FromBody] DTODriverUpdateBus dtoDriverUpdateBus)
        {
            await _driverBLL.UpdateDriverBus(dtoDriverUpdateBus);
            return Ok(new ApiResponseSucess<string>("", "Driver Bus Updated Successfully"));
        }
        ...
        public async Task<ActionResult<ApiResponseSucess<string>>> UpdatePassword(DTOUpdatePassword dtoUpdatePassword)
        {
            ...
            return Ok(new ApiResponseSucess<string>("", "Driver Password Updated Successfuly"));
        }
