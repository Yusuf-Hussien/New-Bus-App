...
window.tripCompletionStatus = JSON.parse(
...
function toggleTripCompletion() {
  window.tripCompletionStatus.isCompleted =
    !window.tripCompletionStatus.isCompleted;
  ...
  localStorage.setItem(
    ...
    JSON.stringify(window.tripCompletionStatus)
  );
            ...
            <div class="modal">
                ...
                <div class="modal-body">
                    <form id="updateProfileForm" class="update-form">
                        ...
                        <!-- الحقول المخفية للبيانات غير القابلة للتعديل -->
                        <input type="hidden" id="email" value="${profile.email}">
                        <input type="hidden" id="phone" value="${profile.phone}">
                        <input type="hidden" id="plateNoBus" value="${profile.plateNoBus}">
                        <input type="hidden" id="createdByAdminName" value="${profile.createdByAdminName}">
...
function handleUpdateProfile(e) {
  ...
  // الحصول على البيانات الحالية قبل التحديث
  const currentProfile = getProfileData();

  // قراءة الحقول القابلة للتعديل فقط
  ...
  // قراءة الحقول المخفية (القيم الأصلية)
  const email =
    document.getElementById("email")?.value.trim() || currentProfile.email;
  const phone =
    document.getElementById("phone")?.value.trim() || currentProfile.phone;
  const plateNoBus =
    document.getElementById("plateNoBus")?.value.trim() ||
    currentProfile.plateNoBus;
  const createdByAdminName =
    document.getElementById("createdByAdminName")?.value.trim() ||
    currentProfile.createdByAdminName;

  // التحقق من الحقول المطلوبة (القابلة للتعديل فقط)
  ...
  const profileData = {
    ...
    // الحقول غير القابلة للتعديل - نحتفظ بقيمها الأصلية
    email: currentProfile.email || email,
    phone: currentProfile.phone || phone,
    plateNoBus: currentProfile.plateNoBus || plateNoBus,
    createdByAdminName: currentProfile.createdByAdminName || createdByAdminName,
